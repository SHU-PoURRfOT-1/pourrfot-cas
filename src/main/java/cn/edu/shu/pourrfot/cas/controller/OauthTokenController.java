...
import cn.edu.shu.pourrfot.cas.model.*;
...
public class OauthTokenController {
  ...
  @ApiResponses({@ApiResponse(code = 403, message = "Wrong certification"), @ApiResponse(code = 200, message = "authorize successful, redirecting..."),})
  public ResponseEntity<?> authorize(@Valid @NotNull @RequestBody OauthAuthorizationCodeRequest oauthAuthorizationCodeRequest) {
    ...
    return user == null ? ResponseEntity.status(HttpStatus.FORBIDDEN)
      .body(OauthAuthorizationCodeResponse.builder()
        .message("Wrong certification")
        .build()) :
      ResponseEntity.ok(OauthAuthorizationCodeResponse.builder()
  ...
  public ResponseEntity<OauthTokenResponse> createToken(@NotNull @Valid @RequestBody OauthTokenRequest oauthTokenRequest) {
    ...
    if (isIllegalClient(clientId, clientSecret)) {
      return ResponseEntity.status(HttpStatus.FORBIDDEN)
        .body(OauthTokenResponse.builder()
